...
import RewardImage from "./reward_image.svg";
...
const Page = () => {
    ...
    const calculateRewardsPerPage = () => {
        if (window.innerWidth <= 420) {
            return rewardsLength; // Adjust this value based on your preference
        } else {
            return 4; // Default number of items per page
        }
    };
    const rewardsPerPage = calculateRewardsPerPage();
    console.log(rewardsPerPage);
                    ...
                    {loadRewards.map((reward, i) => (
                        <article key={i} className={styles.reward}>
                            ...
                            <div className={styles.rewardRight}>
                                ...
                                <p className={styles.rewardInfo}>{window.innerWidth <= 420 ? <h4>type:</h4> : <b>type:</b>} {reward.reward_type}</p>
                                <p className={styles.rewardInfo}>{window.innerWidth <= 420 ? <h4>prijs:</h4> : <b>prijs:</b>} {reward.prijs}</p>
                                <p className={styles.rewardInfo}>{window.innerWidth <= 420 ? <h4>info:</h4> : <b>info:</b>} {reward.info}</p>
                            </div>
                        </article>
                    ))}
                </div>
